...
| [zerotheta-evm](https://github.com/SMC17/zerotheta-evm) (was zeth) | **D — Type-I fix + 2-round differential validation (2026-05-15)** | v1.0.0 | mainnet-compatible posture **SUSPENDED** pending BN256 math fix + leftover-gas-return fix + block-replay re-validation | `compiled` + `unit-tested` (411/414 under canonical YP + post-gas-fix) + `audited` (bug found + fixed + 2-round differential measured in-session) + `differential-tested-post-fix` (PyEVM-Berlin re-run shows 10 residual discrepancies, all named in 3 categories) | EVM in Zig. Same-session audit-fix-measure-fix-measure loop: [PR #23](https://github.com/SMC17/zerotheta-evm/pull/23) opens with the stack-pop convention audit doc + posture suspension. [PR #24](https://github.com/SMC17/zerotheta-evm/pull/24) lands round 1 (12-handler stack fix + 15 test-vector updates) and round 2 (G_copy = 3-gas/word fix on 4 *COPY sites + 3 test-expectation updates). Differential against py-evm in a Python venv: **26 → 20 → 10**. ZERO residual non-commutative arithmetic divergences; ZERO residual canonical-path gas offsets. Residual 10 = 2 BN256 math bugs (G1+G1 / G1*2 — load-bearing correctness, highest residual priority) + 3 leftover-gas-return on precompile failure (original "984390" pattern, now confined) + 2 error-path return-data diffs + 3 harness false-positives. Full audit chain in `POST_FIX_DIFFERENTIAL_REPORT.md`. **The canonical case study of audit-fix-validate-fix-validate in a single session.** |
