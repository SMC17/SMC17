...
| [zeth](https://github.com/SMC17/zeth) | **D — open Type-I correctness finding (2026-05-15)** | v1.0.0 | mainnet-compatible posture **SUSPENDED** | `compiled` + `unit-tested-self-consistent` (test suite codifies the bug) + `audited` (the bug is now documented) | EVM in Zig. Recursive self-audit caught a systematic stack-pop-order inversion across all non-commutative opcodes — SUB, DIV, MOD, ADDMOD, MULMOD, SDIV, SMOD, EXP, LT, GT, SLT, SGT all compute `second OP top` where Yellow Paper §H.2 specifies `top OP second`. Hand-written test vectors in `validation/comparison_tool.zig` and `src/evm/comprehensive_test.zig` encode the inversion, so the green CI suite is internally consistent but is NOT a correctness signal. Documented in [`STACK_CONVENTION_AUDIT.md`](https://github.com/SMC17/zeth/blob/main/STACK_CONVENTION_AUDIT.md). The PyEVM-Berlin diff fuzz's earlier "26 precompile divergences" (PC02-PC09) is likely a misattribution of this first-order bug — task #26 is downstream of task #34 (the fix). The earlier EIP-2929 CALL-base fix in PR #21 stands (orthogonal). **This is exactly the kind of vanity-1.0 the audit register was built to surface.** |
