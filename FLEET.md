...
| [zeth](https://github.com/SMC17/zeth) | **D — Wave-3 PR open** | v1.0.0 | [PR #21](https://github.com/SMC17/zeth/pull/21) awaiting CI + merge | `unit-tested` + `differential-tested` (EIP-2929 corrected; reference corpus expanded by 11 cases) | EVM in Zig. PR #21 ships: EIP-2929 +700 base dropped from all 4 CALL family opcodes (`opCall`/`opStaticCall`/`opCallCode`/`opDelegateCall`); Class A parseU256Hex odd-length pad fix; differential-fuzz panic softener dropped (opCoinbase fixed); 11 more opcode-coverage cases; README softened to honest evidence vocab. **Stealth raise-tide**: full Zig 0.14 → 0.16 migration (GeneralPurposeAllocator → DebugAllocator, ArrayList(T) → array_list.Managed(T), Init.Minimal main, Io threading, callconv .C → .c). The differential-fuzz substrate caught the EIP-2929 violation in zeth's own code — the substrate doing its job is the win. |
