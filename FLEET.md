...
| [zerotheta-evm](https://github.com/SMC17/zerotheta-evm) (was zeth) | **D — Type-I correctness fix shipped, awaiting merge (2026-05-15)** | v1.0.0 | mainnet-compatible posture **SUSPENDED** pending block-replay re-validation | `compiled` + `unit-tested` (411/414 under canonical YP semantics) + `audited` (the bug found + fixed in-session) | EVM in Zig. Recursive self-audit caught a systematic stack-pop-order inversion across SUB, DIV, MOD, ADDMOD, MULMOD, SDIV, SMOD, EXP, LT, GT, SLT, SGT — every non-commutative opcode computed `second OP top` where Yellow Paper §H.2 specifies `top OP second`. Audit + fix shipped same day: [PR #23](https://github.com/SMC17/zerotheta-evm/pull/23) opens with the audit doc and suspends the posture; [PR #24](https://github.com/SMC17/zerotheta-evm/pull/24) lands the 12-handler fix + 15 test-vector updates with all-green local runs. The PyEVM-Berlin diff fuzz match-rate should jump sharply on the post-merge CI; task #26's "26 precompile divergences" claim is now downstream of this first-order fix and should be re-evaluated against a clean baseline. **The canonical case study: a fully-green test suite is not a correctness signal when the tests share the bug. Only a differential against an independent reference can catch this class of error — and zeth had the right design (PyEVM diff fuzz), but the signal was misattributed to gas accounting for a release cycle.** |
