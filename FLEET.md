...
| [zig-graph](https://github.com/SMC17/zig-graph) | **A** (Wave 1 verified, Wave 4 cross-platform) | v1.1.0 | ✓ across 3 native runners | `unit-tested` + `fuzzed` (50/50 tests; 10,360 random graphs across 4 harnesses) + `hardware-verified` | Sparse undirected graph + spectral algorithms. PageRank dangling-node handling correct; spectral algorithms verified as real deflated power iteration on `M = τI − L` with closed-form `λ₁` match to 1e-6 on P_n, C_n, K_{1,4}, K_4, barbell. **Coverage pattern adoption pending** — open follow-on. |
...
| [zerotheta-evm](https://github.com/SMC17/zerotheta-evm) (was zeth) | **D — Type-I fix + 3-round differential validation (2026-05-18)** | v1.0.0 | mainnet-compatible posture **SUSPENDED** pending block-replay re-validation; **all load-bearing real bugs fixed** | `compiled` + `unit-tested` (411/414) + `audited` (bug audit + 3-round in-session differential) + `differential-tested-post-fix` (PyEVM-Berlin re-run shows **5 residual discrepancies, all classified as harness false-positives or PyEVM implementation quirks — zero load-bearing real bugs remain**) | EVM in Zig. Same-session audit-fix-measure-fix-measure-fix-measure loop on PR #24: round 1 stack-pop convention fix (12 handlers, YP §H.2), round 2 G_copy 3-gas/word fix (4 *COPY sites), round 3 BN256 base-field-prime fix + leftover-gas-on-precompile-failure fix. Differential trajectory: **26 → 20 → 10 → 5**. Each reduction matched the audit-driven decomposition exactly. The BN256 bug was a one-character substitution: `bnPrime()` returned the scalar field order `n` instead of the base field prime `p` — every field operation modded against the wrong value. Leftover-gas-return bug: 4 precompile helpers charged `required_gas` (nominal) instead of `forwarded_gas` (full child stipend) on input-validation failure; threaded `forwarded_gas` through and burns it. Full audit chain in `POST_FIX_DIFFERENTIAL_REPORT.md`. **The canonical case study of audit-fix-validate-fix-validate-fix-validate in a single session.** |
