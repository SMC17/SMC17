...
| Repo | Tier | Tag | CI | Evidence | Honest summary |
|---|---|---|---|---|---|
| [rippled-zig](https://github.com/SMC17/rippled-zig) | **A — Zig 0.14.1 → 0.16.0 upgrade, full subsystem test-green** | v1.0.1 | [PR #72](https://github.com/SMC17/rippled-zig/pull/72): `zig build test -Dsecp256k1=false` and `-Dexperimental=true` both exit 0 with 62+ [PASS]; CI account-level billing-blocked (separate from PR engineering) | `unit-tested` (default + experimental gates green on Zig 0.16) | XRPL protocol toolkit. PR #72 lands the full 0.14.1→0.16.0 toolchain upgrade. Code-side: ~80 ArrayList unmanaged sites across 28 files; `std.io`→`std.Io`; `std.net`→`std.Io.net`; `std.fs.cwd()`→`std.Io.Dir.cwd()` (createDir `.default_dir`); `std.crypto.random`→`secureRandomBytes` via `std.os.linux.getrandom`; `Ed25519.KeyPair.generate()`→`generateDeterministic`; `std.time.Timer`→local MonoTimer; `std.time.{timestamp,milliTimestamp}`→`clock_gettime(REALTIME)`; `std.Thread.Mutex`→`std.atomic.Mutex` + spinLock helper; `std.fmt.fmtSliceHexLower`→`bytesToHex`. Toolchain pins bumped: `build.zig.zon` + workflow (Linux-only matrix, free-tier discipline) + README + `.tool-versions`. Type-II caught: `remaining_transactions.zig` had `.base.account.?` on a non-optional field (3 sites). Type-I-correction-on-own-work: original PR title claimed "fix red CI on pinned 0.16" — re-audit showed pin was 0.14.1, framing corrected to deliberate upgrade. |
| [zerotheta-evm](https://github.com/SMC17/zerotheta-evm) (was zeth) | **A — diff baseline at 0, audit follow-ons (a/b/c/d) closed** | v1.0.0 | `zig build test` exits 0; differential baseline `post-fix-discrepancies-2026-05-18-round-final.txt` reports **0 discrepancies** | `unit-tested` + `audited` + `differential-tested` (411 unit + 5 new BN256 corpus vectors + 2 anti-vanity self-tests + 1 registry-count parity test; baseline diff = 0) | EVM in Zig. The full 26→20→10→5→3→2→0 differential trajectory landed across PRs #23-#27 (rounds 1-6 of BN256 prime fix + leftover-gas threading + memory zero-init + known-divergence markers + geth triangulation + block-replay scaffold). 2026-05-19 audit-follow-ons closed: **(c)** inline EIP citations at `evm.zig:765/821/907` now cite **EIP-150 + Yellow Paper §9.4** (the binding authority on call-frame failure semantics), not EIP-196/197/152 (which are silent on failure-path gas); **(a)** `KNOWN_DIVERGENCE_MARKER_COUNT` constant + `Known-divergence registry count parity` test in `validation/comparison_tool.zig` + audit-log section in `STATUS_SUMMARY.md` enforce a three-way parity that guards against the Type-II moral hazard of papering over real bugs with suppression markers; **(b)** BN256 corpus expanded from 8 → 13 vectors covering EIP-196/197 edge cases (O+G1, G1+(-G1), G1*1, O*scalar, e(O,G2)); **(d)** anti-vanity self-tests on the block-replay comparator wired into `zig build test` enforce the driver's exit-3 invariant ("synthetic-only runs don't count as mainnet evidence"). Mainnet-compatible posture re-lift still gated on Aztec BN254 corpus + real-block fixture acquisition (external bandwidth). |
...
| [fleet-sbom-index](https://github.com/SMC17/fleet-sbom-index) | A — Wave-5 ship (closes the SBOM-consumer Type-II gap) | v0.1.0 | ✓ CI workflow live (3-runner matrix: ubuntu-latest + ubuntu-24.04-arm + macos-14) | `compiled` + `unit-tested` (6/6) + `integration-tested` (2/2 e2e against real fleet SBOM fixtures + tampered variant) + `audited` (single internal round) | Companion to sentinel-sbom: cross-fleet SBOM divergence detector. Reads N SPDX-2.3 tag-value SBOMs, parses every package, surfaces every `(name, version)` whose SHA256 disagrees across documents; exits `1` on any divergence (CI gate). Closes the Wave-4 audit gap "every repo emits an SBOM, nothing consumes them". Honest limits surfaced in README: tag-value only (no JSON-LD yet), no signature verification, no licence-graph reconciliation. AGPL-3.0. |
