...
| [zerotheta-evm](https://github.com/SMC17/zerotheta-evm) (was zeth) | **D — Type-I fix + post-fix differential measured (2026-05-15)** | v1.0.0 | mainnet-compatible posture **SUSPENDED** pending BN256 fix + gas-offset fix + block-replay re-validation | `compiled` + `unit-tested` (411/414 under canonical YP) + `audited` (bug found + fixed + post-fix differential measured in-session) + `differential-tested-post-fix` (PyEVM-Berlin re-run shows 20 residual discrepancies, sorted into 3 named root causes) | EVM in Zig. Recursive self-audit caught a systematic stack-pop-order inversion across SUB/DIV/MOD/ADDMOD/MULMOD/SDIV/SMOD/EXP/LT/GT/SLT/SGT — every non-commutative opcode computed `second OP top` where Yellow Paper §H.2 specifies `top OP second`. Audit + fix + post-fix validation shipped same day: [PR #23](https://github.com/SMC17/zerotheta-evm/pull/23) (audit doc + posture), [PR #24](https://github.com/SMC17/zerotheta-evm/pull/24) (12-handler + 15-test fix), [POST_FIX_DIFFERENTIAL_REPORT.md](https://github.com/SMC17/zerotheta-evm/blob/fix-stack-convention-inversion/POST_FIX_DIFFERENTIAL_REPORT.md) (py-evm differential, post-fix discrepancy count 26 → 20). The 20 residual discrepancies sort into 3 named root causes: (a) BN256 elliptic-curve math wrong on G1+G1 and G1*2 — REAL math bug, highest priority; (b) 11 precompile gas-cost offsets (+2/+8/+12/+24) — single likely root cause, ~1-line fix; (c) 3 cases of leftover-gas-return on failed precompiles — the original "984390" pattern confined to 3 cases, not 26. **The canonical case study + the validation that proved the recursive-audit theory in the same session.** |
...
**Headline finding (2026-05-15) + validation in the same session:** zeth ships a systematic stack-pop-order inversion across SUB, DIV, MOD, ADDMOD, MULMOD, SDIV, SMOD, EXP, LT, GT, SLT, SGT — every non-commutative arithmetic, comparison, and exponent opcode. The implementation computes `second OP top`; the Yellow Paper specifies `top OP second`. The harness test vectors in `validation/comparison_tool.zig` codify the inversion so the green CI passes despite the bug. Audited in `zeth/STACK_CONVENTION_AUDIT.md` (PR #23), fixed in PR #24 (12 handlers + 15 test vectors, all-green local), then **the same session installed py-evm in a Python venv and re-ran the differential against the fixed branch** — verbatim report in `POST_FIX_DIFFERENTIAL_REPORT.md`. Discrepancy count 26 → 20, with ZERO residual divergences in the non-commutative arithmetic family and the remaining 20 sorting cleanly into 3 named root causes (BN256 math, gas offset, leftover-gas-return) + 2 harness artifacts. The recursive-audit theory predicted "most of the 26 are downstream of the stack-pop bug" — the measurement bears that out.
